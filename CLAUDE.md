...
**Purpose:** A domain-agnostic digital twin engine. Given any dataset and a plain-text problem description, the engine automatically generates schema contracts, fits a probabilistic model, optimises decisions via RL, and delivers an interactive dashboard — without hardcoding any domain-specific logic.   
**Stage:** Phase 1 — validating the full pipeline with a manufacturing dataset. Phase 2 will make OR fully dynamic.
...
│   ├── problem_description.json   ← Human-provided per run. Only manual artifact.
...
| **OR** | DB profiling, causal process graph, schema modelling | `context/problem_description.json` (reads), `context/schema.json`, `context/model_spec.json` (writes) |
...
## 9. Engine Design Rules

This system is a domain-agnostic engine, not a domain-specific application.
These rules are non-negotiable across all agents and all phases.

### 9.1 No Hardcoded Domain Knowledge
No agent may hardcode domain-specific variable names, table names, column names,
or model parameters anywhere in code or prompts.
All domain knowledge lives exclusively in `context/*.json`.
Code reads from contracts — it never defines them.

### 9.2 The Only Manual Input
`context/problem_description.json` is the single artifact a human fills per run.
Everything else in `context/` is generated by agents from that file and the dataset.

### 9.3 Phase Awareness
Agents must check `session_log.json → engine_phase` at session start:

- `phase_1`: schema.json may be hand-authored for the pilot dataset. Hardcoding is
  permitted only here, only temporarily, and must be flagged with a `# PHASE_1_ONLY`
  comment so it can be found and removed in Phase 2.
- `phase_2`: OR auto-generates schema.json from the dataset. No hardcoding anywhere.
- `phase_3`: UX auto-adapts to any schema.json. No hardcoded variable names in UI.

### 9.4 The Generalisation Test
Before finishing any session, each agent asks:
*"Would this code still work if I swapped in a logistics dataset tomorrow?"*
If the answer is no, the hardcoded assumption must be extracted into context/*.json.
